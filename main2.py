...
if api_status_ok:
    ...
    def extract_data(url: str, is_competitor: bool = False) -> Optional[dict]:
        try:
            ...
            # Gunakan scrape_url dengan agent FIRE-1 dan json_options
            scrape_result = app.scrape_url(
                ...
                agent={
                    'model': 'FIRE-1',
                    'prompt': """
                    Navigasi dan ekstrak informasi dengan melakukan:
                    1. Cari dan klik tombol navigasi yang relevan (seperti "Harga", "Produk", "Fitur", "Teknologi", "Tentang Kami")
                    2. Scroll halaman untuk menemukan semua informasi penting
                    3. Jika ada popup, tutup atau klik tombol yang relevan
                    """
                }
            )
            
            if not scrape_result or not hasattr(scrape_result, 'success') or not scrape_result.success:
                if hasattr(scrape_result, 'error') and 'Insufficient credits' in str(scrape_result.error):
            ...
            if hasattr(scrape_result, 'data') and 'json' in scrape_result.data:
                ...
                return {
                    ...
                    "tech_stack": data.get('tech_stack', [])[:3],
